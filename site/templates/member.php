	...
	<div id="profile"
		...
		<div class="p-4 md:p-12 text-center lg:text-left">
			...
			<div class="block lg:hidden rounded-full shadow-xl mx-auto -mt-16 h-48 w-48 bg-cover bg-center"
				style="background-image: url('<?= $page->cover()->url() ?>')"></div>

			<h1 class="text-3xl font-bold pt-8 lg:pt-0"><?= $page->name() ?></h1>
			<div class="mx-auto lg:mx-0 w-4/5 pt-3 border-b-2 border-gold-500 opacity-25"></div>
			<?php if ($page->role()->exists()): ?>
			...
			<?php endif ?>
			<p class="pt-8 text-sm"><?= $page->description() ?></p>

			<div class="pt-12 pb-8">
				<button class="bg-gold-700 hover:bg-green-900 text-white font-bold py-2 px-4 rounded-full">
					Get In Touch
				</button>
			</div>
			<?php if ($page->teams()->exists()): ?>
			<div class="mt-6 pb-16 lg:pb-0 w-4/5 lg:w-full mx-auto flex flex-wrap items-center justify-between">
				<?php foreach($page->teams()->split(',') as $team): ?>
					<?= var_dump($team->split(',')) ?>
					<span class="bg-blue-100 text-blue-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">#<?= $team ?></span>
				<?php endforeach ?>
			</div>
			<?php endif ?>
